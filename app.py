import os, io, csv, json, base64, traceback, tempfile, shutil, threading, time, uuid
...
def init_db():
    ...
    try:
        ...
        x.execute("ALTER TABLE training_state ADD COLUMN IF NOT EXISTS run_id TEXT")
        x.execute("ALTER TABLE training_state ADD COLUMN IF NOT EXISTS server_id TEXT")
        x.execute("ALTER TABLE training_state ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ")
        x.execute("ALTER TABLE training_state ADD COLUMN IF NOT EXISTS finished_at TIMESTAMPTZ")
...
def mark_interrupted_on_startup():
    c = db(); x = c.cursor()
    try:
        x.execute("""UPDATE training_state SET status='interrupted', message='Training interrupted because the Render process restarted. Start a new training run manually.', progress=0, updated_at=NOW(), finished_at=NOW() WHERE id=1 AND status IN ('preparing','training','saving')""")
        if x.rowcount: print('NEERIKA TRAINING: previous active run marked INTERRUPTED after process restart.')
        c.commit()
    finally:
        x.close(); c.close()
...
def acquire_training_lock():
    ...
    cur = conn.cursor(); cur.execute('SELECT pg_try_advisory_lock(%s) AS locked', (TRAINING_LOCK_KEY,)); row=cur.fetchone(); cur.close()
...
def release_training_lock(conn=None):
    ...
    if not conn: return
    try:
        cur=conn.cursor(); cur.execute('SELECT pg_advisory_unlock(%s)', (TRAINING_LOCK_KEY,)); cur.close()
    except Exception: pass
    try: conn.close()
    except Exception: pass
    if conn is TRAINING_LOCK_CONN: TRAINING_LOCK_CONN=None


def claim_training_run():
    global TRAINING, TRAINING_RUN_ID
    lock_conn=acquire_training_lock()
    if not lock_conn: return None
    run_id=uuid.uuid4().hex
    c=db(); x=c.cursor()
    try:
        x.execute("""UPDATE training_state SET status='preparing', message='Training accepted. Preparing dataset...', progress=1, updated_at=NOW(), run_id=%s, server_id=%s, started_at=NOW(), finished_at=NULL WHERE id=1 AND status NOT IN ('preparing','training','saving') RETURNING id""", (run_id,SERVER_INSTANCE_ID))
        if not x.fetchone():
            c.rollback(); release_training_lock(lock_conn); return None
        c.commit(); TRAINING=True; TRAINING_RUN_ID=run_id; return run_id
    except Exception:
        c.rollback(); release_training_lock(lock_conn); raise
    finally:
        x.close(); c.close()
...
def training_worker(run_id, lock_conn):
    ...
    try:
        ...
        set_training('training',f'Dataset ready: {usable} images. Starting YOLO {TRAIN_EPOCHS} epochs...',18,run_id)
        ...
        def on_epoch_end(trainer):
            try:
                ...
                set_training('training',f'Training epoch {epoch}/{total}...',progress,run_id)
                print(f'NEERIKA TRAINING: epoch {epoch}/{total} -> {progress}%')
            except Exception: traceback.print_exc()
        model.add_callback('on_train_epoch_end',on_epoch_end)
        set_training('training',f'Training YOLO for {TRAIN_EPOCHS} epochs... Epoch 0/{TRAIN_EPOCHS}',20,run_id)
        print(f'NEERIKA TRAINING START: run={run_id}, {TRAIN_EPOCHS} epochs, workers=0, batch=4')
        ...
        set_training('saving','Training complete. Saving best.pt...',93,run_id)
        os.makedirs(os.path.dirname(MODEL_PATH),exist_ok=True); shutil.copy2(best,MODEL_PATH)
        os.makedirs(os.path.dirname(MODEL_BACKUP_PATH),exist_ok=True); shutil.copy2(best,MODEL_BACKUP_PATH)
        print('NEERIKA TRAINING: best.pt copied to persistent app paths.')
        set_training('saving','best.pt saved locally. Saving best.pt to Supabase...',96,run_id)
        save_model_db(MODEL_PATH)
        print('NEERIKA TRAINING: best.pt saved to Supabase.')
        with MODEL_LOCK: MODEL=YOLO(MODEL_PATH); MODEL_ERROR=''
        set_training('completed',f'Training completed successfully. {usable} images, {TRAIN_EPOCHS} epochs. best.pt saved.',100,run_id,finished=True)
        print(f'NEERIKA TRAINING COMPLETE: run={run_id}')
    except Exception as e:
        traceback.print_exc()
        try: set_training('error','Training error: '+str(e),0,run_id,finished=True)
        except Exception: traceback.print_exc()
    finally:
        if root: shutil.rmtree(root,ignore_errors=True)
        TRAINING=False; TRAINING_RUN_ID=None; release_training_lock(lock_conn)
...
def start_training_request():
    ...
    try:
        threading.Thread(target=training_worker,args=(run_id,lock_conn),daemon=True).start()
        return run_id
    except Exception:
        try: set_training('error','Training worker could not be started.',0,run_id,finished=True)
        finally:
            global TRAINING, TRAINING_RUN_ID
            TRAINING=False; TRAINING_RUN_ID=None; release_training_lock(lock_conn)
        raise
...
def training():
    c=db(); x=c.cursor(); x.execute('SELECT status,message,progress,updated_at,run_id FROM training_state WHERE id=1'); s=x.fetchone() or {'status':'idle','message':'Ready','progress':0,'updated_at':None,'run_id':None}; x.close(); c.close()
    rows=dataset_rows()
    ...
    body=r'''<div class="card"><h2>YOLO Training Dataset</h2><p class="small">Upload images. After upload, use the annotation API or your existing annotation workflow to save BUCKET_LOADED boxes.</p><div class="row"><label>Image</label><input id="file" type="file" accept="image/*"></div><button onclick="upload()">Upload Image</button><div id="up" class="status"></div></div><div class="card"><h3>Training Status</h3><p>Status: <b id="st">__STATUS__</b></p><p id="ms">__MESSAGE__</p><p class="small">Last update: <span id="upd">__UPDATED__</span></p><div class="progress"><div id="bar" class="bar" style="width:__PROGRESS__%"></div></div><br><button id="startBtn" class="success" onclick="startTrain()">Start Training</button><button class="secondary" onclick="poll()">Refresh Status</button></div><div class="card"><h3>Dataset</h3><table><tr><th>ID</th><th>Filename</th><th>Annotations</th><th>Action</th></tr>__ROWS__</table></div><script>
let pollTimer=null; const activeStates=['preparing','training','saving'];
function setStatus(d){st.innerText=d.status||'idle';ms.innerText=d.message||'';bar.style.width=Number(d.progress||0)+'%';upd.innerText=d.updated_at||'-';const active=activeStates.includes((d.status||'').toLowerCase());startBtn.disabled=active;startBtn.innerText=active?'Training Running...':'Start Training';if(active){if(pollTimer)clearTimeout(pollTimer);pollTimer=setTimeout(poll,2000)}}
async function upload(){let f=file.files[0];if(!f){alert('Choose image');return}let rd=new FileReader();rd.onload=async()=>{let q=await fetch('/api/dataset/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:f.name,mime_type:f.type,data:rd.result})});let d=await q.json();up.innerText=d.message||d.error||'';if(q.ok)setTimeout(()=>location.reload(),500)};rd.readAsDataURL(f)}
async function delimg(id){if(!confirm('Delete image?'))return;let q=await fetch('/api/dataset/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});let d=await q.json();if(!q.ok){alert(d.error);return}location.reload()}
async function startTrain(){startBtn.disabled=true;try{let q=await fetch('/api/training/start',{method:'POST',headers:{'Content-Type':'application/json'}});let d=await q.json();if(!q.ok)alert(d.error||'Training could not be started.');await poll()}catch(e){alert('Connection error: '+e.message);await poll()}}
async function poll(){try{let q=await fetch('/api/training/status',{cache:'no-store'});let d=await q.json();setStatus(d)}catch(e){ms.innerText='Status connection error. Retrying...';if(pollTimer)clearTimeout(pollTimer);pollTimer=setTimeout(poll,3000)}}
poll();
</script>'''.replace('__STATUS__',esc(s['status'])).replace('__MESSAGE__',esc(s['message'])).replace('__PROGRESS__',esc(s['progress'])).replace('__UPDATED__',esc(s['updated_at'] or '-')).replace('__ROWS__',tr)
    ...
    def do_GET(self):
        try:
            ...
            if p=='/api/training/status':
                c=db();x=c.cursor();x.execute('SELECT status,message,progress,updated_at,run_id,server_id,started_at,finished_at FROM training_state WHERE id=1');r=x.fetchone();x.close();c.close();return json_out(self,r or {'status':'idle','message':'','progress':0})
            if p=='/api/history':
    ...
    def do_POST(self):
        try:
            ...
            if p=='/api/training/start':
                ...
                if not run_id:return error_out(self,'Training is already running. Wait for the current run to finish.',409)
                return json_out(self,{'message':'Training started.','run_id':run_id},202)
            return error_out(self,'Not found.',404)
